...
def get_nested_dict_value(dct: dict,
                          keypath: str,
                          default=None,
                          separator: str = ".") -> Any:
...
def parse_tracking_response(response: dict,
                            include_original: bool = False) -> dict:
    """Parse tracking data"""
    output = {}

    error: str | None = get_nested_dict_value(
        response, "Fault.detail.Errors.ErrorDetail.PrimaryErrorCode")
    if error is not None:
        output = {"external_error": error}

    package_data: dict | list[dict] = get_nested_dict_value(
        response, "TrackResponse.Shipment.Package")
    if package_data is not None:
        data = {}

        shipment_references = set()
        shipment_reference_data: list = get_nested_dict_value(
            response, "TrackResponse.Shipment.ReferenceNumber")
        ...
        for package in package_data:
            ...
            most_recent_activity_status = most_recent_activity["Status"][
                "Description"]
...
def parse_address_validation_response(response: dict,
                                      include_original: bool = False) -> dict:
    ...
    errors: list[str] | None = get_nested_dict_value(response,
                                                     "response.errors")
    ...
    no_candidates_indicator: str | None = get_nested_dict_value(
        response, "NoCandidatesIndicator")
    valid_address_indicator: str | None = get_nested_dict_value(
        response, "ValidAddressIndicator")

    if no_candidates_indicator is not None:
        output["status"] = "Address unknown, No candidates"

    address_classification: str | None = get_nested_dict_value(
        response, "AddressClassification.Description")
    ...
    if output["status"] is None:
        ...
        street_address = get_nested_dict_value(first_candidate,
                                               "AddressKeyFormat.AddressLine")
        output["street_address"] = " ".join(list(street_address))

        output["region"] = get_nested_dict_value(first_candidate,
                                                 "AddressKeyFormat.Region")
...
def parse_time_in_transit_response(response: dict,
                                   include_original: bool = False) -> dict:
    """Parse time in transit data"""
    alert: str = get_nested_dict_value(
        response, "TimeInTransitResponse.Response.Alert.Description")
    ...
    service_data = get_nested_dict_value(
    ...
    for service in service_data:
        ...
        pickup_date = get_nested_dict_value(service,
                                            "EstimatedArrival.Pickup.Date")
        pickup_time = get_nested_dict_value(service,
                                            "EstimatedArrival.Pickup.Time")
        pickup_datetime = datetime.strptime(pickup_date + pickup_time,
                                            "%Y%m%d%H%M%S")

        arrival_date = get_nested_dict_value(service,
                                             "EstimatedArrival.Arrival.Date")
        arrival_time = get_nested_dict_value(service,
                                             "EstimatedArrival.Arrival.Time")
        arrival_datetime = datetime.strptime(arrival_date + arrival_time,
                                             "%Y%m%d%H%M%S")

        business_days_in_transit = get_nested_dict_value(
            service, "EstimatedArrival.BusinessDaysInTransit")
        day_of_week = get_nested_dict_value(service,
                                            "EstimatedArrival.DayOfWeek")
        customer_center_cutoff = datetime.strptime(
            get_nested_dict_value(service,
                                  "EstimatedArrival.CustomerCenterCutoff"),
            "%H%M%S").time()
        ...
        output["services"].append({
            ...
            "business_days_in_transit":
            business_days_in_transit,
            "day_of_week":
            day_of_week,
            "customer_center_cutoff":
            str(customer_center_cutoff),
        })
